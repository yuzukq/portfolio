...
export const products: Product[] = [
  {
    ...
    description: "長期インターンではNext.jsを用いたフロントエンド開発を行いました．モダンなフレームワークを利用する中で，コンポーネント分割の粒度，保守・拡張性を考慮した設計思想，OSSデザインシステムの効率的な活用方法について学びました．これらの知見を活かした振り返りもかね，これまでの大学生活の活動を可視化することを目的に，当プロダクトの開発を行いました．また，ブログを管理するプロジェクトとしての役割も担っており，rehypeを用いたマークダウンパースによりNext.js上での静的ページ生成を実現しています．これにより，git上での記事管理とデプロイの自動化を実現しています．",
    features: [
      "レスポンシブデザイン", 
      "アニメーション効果",
      "Markdownによるブログ記事管理",
